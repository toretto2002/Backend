...
public class ContattoController {
	...
	@PostMapping
	public ResponseEntity<?> creaContatto(@RequestBody Contatto contact){
		try {
			Contatto c = contattoService.createContatto(contact);
			return new ResponseEntity<Contatto>(c, HttpStatus.CREATED);
		}catch(Exception e) {
			return new ResponseEntity<String>(e.getMessage(), HttpStatus.NOT_FOUND);
		}
	}
